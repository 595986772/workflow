# 论文关键实验结果（2026-08-21 复评版）

> 正式创新仅收敛为 DCC 和 Pairwise PD3QN。任务依赖状态与 makespan-increment
> 奖励是调度器的重要设计，但不单独包装成与两个核心算法并列的新贡献。
...
任务依赖状态和因果 makespan-increment 奖励可在算法设计中保留，但不与 DCC
和 Pairwise PD3QN 并列为独立核心贡献。缓存评分中的依赖加权未获得显著消融支持，
不应被声称为已验证的独立创新。
...
该组仅使用 seeds 51--53，报告均值、样本标准差和原始 seed 点。

| 环境 | 容量方差 | OUR (s) | OUR vs DAOC | OUR vs DDQN + DCC |
|---|---:|---:|---:|---:|
| H0 | 0.16 | 0.4153 | 54.95% | 7.69% |
| H1 | 0.36 | 0.3541 | 64.28% | 20.28% |
| H2 | 0.56 | 0.3547 | 56.09% | 20.98% |
| H3 | 0.96 | 0.4193 | 46.43% | 6.61% |

OUR 在四个已评估容量分布中均取得最低三 seed 平均完成时间，但改善幅度不随方差单调变化，
不作显著性或单调性声称。

## 12. 关键路径时延构成修复

P15-L 基于实现完成时间的关键路径分解处理计算、依赖传输、服务加载和等待时延，不再把并行
分支逐任务相加。在主环境中，OUR 完成时间为 0.2668 s，其中关键路径服务加载约为
0.2014 s，仍是最大构成项。该结果用于解释 DCC 和卸载--副本匹配为何有效，不表示对并行
分支的总工作量分解。

## 13. 开销与 Oracle

- OUR 冻结推理约 0.193 ms/任务决策，DAOC 约 0.0217 ms/决策。
- DCC 的本机 Python 微基准协调时间约为 9.1--9.9 ms/call，紧凑摘要通信量约为 4.85 KB/call。
- OUR 的平均绝对 Oracle gap 约 0.2249 s，说明仍存在优化空间。
- 该 Oracle 为容量感知诊断参考，不是已认证的理论下界。
- 冻结评估不执行在线缓存更新，所以其中缓存决策时间为 0，不能解释为实际协调开销为 0。
- 上述开销只可称为在指定硬件与 Python 实现下“可接受”，不可称为“可忽略”。
...
- 容量异构度：`results/pegasus_pscale/p15_b8_heterogeneity/analysis/`
- 关键路径时延构成：`results/pegasus_pscale/p15_latency_composition_repair/`
